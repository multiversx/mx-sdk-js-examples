...
export class HW {
  ...
  async login() {
    ...
    const addressIndex = parseInt(
      document.getElementById("addressIndexForLogin").value
    );
  ...
  async setAddressIndex() {
    ...
    const addressIndex = parseInt(
      document.getElementById("addressIndexForSetAddress").value
    );
    ...
    displayOutcome(
      `Address has been set: ${await this.hwProvider.getAddress()}.`
    );
  }
  ...
  async signTransaction() {
    ...
    const transactionOptions = guardian
    ...
    const signedTransaction = await this.hwProvider.signTransaction(
      transaction
    );
  ...
  async signTransactions() {
    ...
    const signedTransactions = await this.hwProvider.signTransactions(
      transactions
    );
  ...
  async getGuardian(sender) {
    const guardianData = await this.proxyNetworkProvider.getGuardianData(
      sender
    );
  ...
  async showSignedTransactionsWhenGuarded() {
    const plainSignedTransactions =
      this.walletProvider.getTransactionsFromWalletUrl();
